def backtracking(choice_points, choices, assignable):
    ...
    Source: http://code.activestate.com/recipes/577777-backtracking-template-method/
    """
    ...
    while(not end):
        #forward
        while(  not backtrack ):
            is_assignable = assignable( cp, c, solutions )
            if( is_assignable ):
                solutions[cp] = c
                if( cp==M-1):
...
if __name__=='__main__':
    ...
    def assignable4Colors(nation, color, solutions):
        ...
        neighbors_isdisjoint = neighbors[nation].isdisjoint(sameColor)
        return neighbors_isdisjoint
